...
const Header = ({ setIsAuth }) => {
  ...
  return (
    <div className="sticky top-0 z-90 h-24 flex w-full justify-between items-center md:justify-end px-6 bg-white border-b shadow-lg py-1">
      <div className="w-full lg:w-42 flex gap-x-2 justify-between md:justify-end items-center ">
        <img src={iconLogo} className="cursor-pointer w-50 h-11 " />
        <img
          src={iconDesplegar}
          className={` cursor-pointer self-end mb-4 w-14 p-3 mt-6 ${showMenu
        ...
        />
        {showMenu && (
          <div className="animate__animated border animate__bounceInDown absolute top-22 bg-white right-5 w-40 h-50 md:w-60 flex flex-col justify-center items-center z-99999  rounded-2xl shadow-xl">
            <img src={user} className="w-20 h-20 rounded-full -mt-6" alt="" />
